...
def model_dnnSMJ1(train_data, val_data, test_data):
    ...
    justification_onehotvector = vectorizer_justification.transform(val_data['context']).toarray()
    ...
    justification_onehotvector = vectorizer_justification.transform(test_data['context']).toarray()
...
def model_dnnSMJ2(train_data, val_data, test_data):
    vectorizer_statement = CountVectorizer(binary=True, stop_words=nltk.corpus.stopwords.words('english'),
                                           lowercase=True, min_df=3, max_df=0.9, max_features=3000)
...
def model_lrSMJ(train_data, val_data, test_data):
    vectorizer_statement = CountVectorizer(binary=True, stop_words=nltk.corpus.stopwords.words('english'),
                                           lowercase=True, min_df=3, max_df=0.9, max_features=3000)
